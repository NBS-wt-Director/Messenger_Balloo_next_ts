...
Этот документ сводит всё дерево узлов экосистемы Balloo и ЦФР, объясняет отношения между узлами и содержит короткий rebuild order для восстановления системы.
...
│
├── work-plane (Production)
│   └── work-server ⭐⭐⭐ CRITICAL PRODUCTION
│
└── home-plane (Dev/Backup)
...
- laptop_control УПРАВЛЯЕТ work-server
- Deployment инициируется с laptop_control
- work-server ПРИНИМАЕТ команды

### control-plane → home-plane
- laptop_control УПРАВЛЯЕТ home_aio/home_nas
- Dev deployment на home_aio
- Backup на home_nas

### work-plane → home-plane
- work-server СИНХРОНИЗИРУЕТСЯ с home_nas (backups)
- home_aio MAY синхронизироваться с work-server (code mirror)
...
**Step 2: Восстановить work-plane**
```bash
# 2.1 Provision work-server
# - Установить Linux
...
**Step 3: Восстановить home-plane**
```bash
...
# 3.2 Настроить home_aio (опционально)
# - Установить Linux
# - Настроить Docker
# - Sync с work-server
```
...
# 4.4 Проверить DNS и Cloudflare Tunnel
...
2. **Work Plane:**
   - work-server MUST быть always-on
...
5. **Domains:**
   - balloo.su → work-server
   - центр-фр.рф → work-server
