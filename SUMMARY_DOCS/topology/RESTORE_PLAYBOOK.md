...
### Scenario 1: Loss of work_server (CRITICAL)
...
1. **Provision new node**
   ...
   # Configure hostname: work_server
...
3. **Restore configuration**
   ...
   # Restore .env from encrypted backup (storage node or password manager)
...
6. **Update DNS (if IP changed)**
   ```bash
   # Update DNS records
   # balloo.su → new work_server IP via reverse proxy
   ```
...
2. **Restore access**
   ...
   # Test SSH access to work_server
   ssh work_server
   ```

---

### Scenario 3: Loss of storage node (home_nas)

**Impact:** No backups

**Steps:**

1. **Setup new storage node**
   ```bash
...
- [ ] Docker containers running on work_server
- [ ] Database accessible
