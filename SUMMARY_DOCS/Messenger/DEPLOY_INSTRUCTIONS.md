...
**ВАЖНО: Перед миграцией создайте резервную копию!**
...
### 3. Генерация секретов (если нужно)
...
### Безопасные миграции (не ломают данные)

Добавленные миграции **БЕЗОПАСНЫ** для существующих пользователей:

1. **20260430130442_add_bio_field**
   - Добавляет поле `bio TEXT` (nullable)
   - Все существующие пользователи сохранены
   - Регистрация/вход не сломаны

2. **20260430131150_add_settings_field**
   - Добавляет поле `settings JSONB` (nullable)
   - Все существующие данные сохранены
   - Сообщения/чаты не затронуты

### Что изменено:
- ✅ Добавлены опциональные поля профиля (`bio`, `settings`)
- ✅ Исправлены индексы для `Contact` и `FamilyRelation`
- ✅ НЕТ изменений в обязательных полях
- ✅ НЕТ удаления существующих данных
- ✅ Регистрация и вход работают как прежде
